...
const SleepQualityCard = ({ score }: { score: number }) => (
  <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 h-64 flex flex-col items-center justify-center relative overflow-hidden group">
    ...
    <div className="relative z-10">
      ...
      >
        {score >= 80 ? <Star className="w-16 h-16 text-yellow-300 fill-yellow-300/20" /> :
          score >= 50 ? <Moon className="w-16 h-16 text-purple-300 fill-purple-300/20" /> :
...
const SummarySection = ({ stress, sleep }: { stress: number, sleep: number }) => (
...
const EmotionRadarCard = ({ data }: { data: any[] }) => (
  <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 h-80 relative overflow-hidden">
    <h3 className="text-gray-300 font-medium mb-2 text-center">Emotion Balance</h3>
...
const DreamDetailModal = ({ stat, onClose }: { stat: DailyDreamStat | null, onClose: () => void }) => {
        ...
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-white mb-1">
              {new Date(stat.date).toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'long' })}
            </h3>
            <p className={`text-sm font-medium ${stat.sleepScore >= 3 ? 'text-teal-300' : 'text-gray-400'}`}>
              {stat.sleepScore >= 3 ? "꿀잠 주무셨군요! 🌙" : "잠을 충분히 못 잤어요 ㅠ"}
            </p>
          </div>
          ...
          <div className="w-full bg-white/5 rounded-xl p-4 flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="text-white font-bold capitalize">{EMOTION_LABELS[stat.primaryEmotion]}</div>
            </div>
          </div>
          ...
          {stat.webtoonData ? (
            <div className="w-full space-y-3">
              <div className="aspect-video rounded-xl overflow-hidden relative group cursor-pointer"
                ...
                <img src={stat.webtoonData.thumbnail} alt="Webtoon Thumbnail" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="text-white font-medium flex items-center gap-1">보러가기 <ChevronRight size={16} /></span>
                </div>
              </div>
              <button
                onClick={() => navigate(`/webtoon/${stat.webtoonData!.id}`)}
            ...
            <div className="w-full py-8 text-center bg-white/5 rounded-xl border border-dashed border-white/10">
              ...
              <p className="text-gray-500 text-sm">기록된 꿈 웹툰이 없습니다.</p>
            </div>
          )}
        </div>
      </motion.div>
...
const WeeklyDreamHeatmap = ({ stats, onSelect }: { stats: DailyDreamStat[], onSelect: (stat: DailyDreamStat) => void }) => (
  <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 h-80 flex flex-col">
    <h3 className="text-gray-300 font-medium mb-6 text-center">Weekly Dream Flow</h3>
...
const DetailAnalysisSection = ({ emotions, weekly, onSelectDate }: { emotions: any, weekly: DailyDreamStat[], onSelectDate: (stat: DailyDreamStat) => void }) => {
...
const getInsight = (stress: number, emotions: any) => {
  const dominantEmotion = Object.keys(emotions).reduce((a, b) => emotions[a] > emotions[b] ? a : b) as string
  const stressLevel = stress >= 70 ? 'High' : stress >= 40 ? 'Mid' : 'Low'

  // Logic Mapping Table
  if (stressLevel === 'High' && dominantEmotion === 'anxiety') {
    return {
      message: "현재 마음속에 막연한 **불안감**이 가득 차 있어요. 통제하기 힘든 상황에 대한 압박감이 꿈으로 나타나고 있습니다.",
  ...
  if (stressLevel === 'High' && dominantEmotion === 'angry') {
    return {
      message: "억눌린 **감정의 폭발**이 감지됩니다. 현실에서 표현하지 못한 답답함이 꿈속에서 표출되고 있어요.",
  ...
  if (stressLevel === 'Mid' && dominantEmotion === 'sadness') {
    return {
      message: "**마음의 감기**가 지나가는 중이에요. 상실감이나 아쉬움이 꿈에 묻어납니다. 스스로를 토닥여줄 시간이 필요해요.",
  ...
  if (stressLevel === 'Low' && dominantEmotion === 'peace') {
    return {
      message: "무의식이 매우 **안정된 상태**입니다. 정서 회복력이 훌륭해요! 지금의 평화로운 리듬을 유지하세요.",
  ...
  if (stressLevel === 'Low' && dominantEmotion === 'joy') {
    return {
      message: "**긍정 에너지**가 넘치시네요! 꿈속에서도 활력이 느껴집니다. 이 좋은 기운을 현실에서도 마음껏 펼쳐보세요.",
  ...
  if (dominantEmotion === 'surprise') {
    return {
      message: "변화에 대한 **긴장감**이 감지됩니다. 낯선 환경이나 새로운 도전을 앞두고 있진 않으신가요?",
  ...
  // Default (Balanced)
  return {
    message: "감정의 **균형이 잘 잡혀** 있습니다. 특별히 치우침 없이 건강한 무의식 흐름을 보이고 있어요.",
    action: "가벼운 산책으로 리프레시하기",
    tone: "차분함"
  }
}

const CoachingSection = ({ stress, emotions }: { stress: number, emotions: any }) => {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-2xl font-bold text-white leading-relaxed max-w-2xl"
        dangerouslySetInnerHTML={{ __html: insight.message.replace(/\*\*(.*?)\*\*/g, '<span class="text-purple-300">$1</span>') }}
      />

      <div className="bg-white/5 rounded-xl px-6 py-4 flex items-center gap-3 border border-white/10">
        <Zap className="w-5 h-5 text-yellow-400" />
        <span className="text-gray-200 text-sm font-medium">추천 액션: {insight.action}</span>
      </div>
    ...
    <div className="min-h-full w-full bg-[#0F0C29] text-white flex flex-col py-12 px-4 pb-32 overflow-y-auto scrollbar-hide">
      <div className="max-w-4xl mx-auto w-full">
        ...
        >
          {analysis && (
            <>
              <SummarySection stress={analysis.stressIndex} sleep={analysis.sleepQuality} />
              <DetailAnalysisSection
                emotions={analysis.emotionDistribution}
                weekly={analysis.weeklyStats}
                onSelectDate={setSelectedStat}
              />
              <CoachingSection stress={analysis.stressIndex} emotions={analysis.emotionDistribution} />
            </>
          )}
        </motion.div>
        ...
        <AnimatePresence>
          {selectedStat && (
            <DreamDetailModal stat={selectedStat} onClose={() => setSelectedStat(null)} />
          )}
        </AnimatePresence>
      </div>
    </div>
