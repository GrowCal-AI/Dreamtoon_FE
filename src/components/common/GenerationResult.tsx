...
import { Save, Sparkles, RotateCcw, MessageCircle, Heart, Share2, Download, X, Copy, Check } from 'lucide-react'
...
const GenerationResult = ({
    ...
    return (
        <div className="w-full min-h-screen flex flex-col items-center justify-center relative overflow-hidden text-white">
            ...
            >
                <div className="max-w-[1200px] mx-auto px-5 xl:px-8 py-4 flex items-center justify-between">
                    ...
                    <div className="flex flex-col gap-1 items-start text-left">
                        <span className="text-xs text-purple-400 font-medium">오늘의 꿈 웹툰 완성!</span>
                    ...
                    <div className="flex items-center gap-3">
                        {onSave && (
                            !isSaved ? (
                                ...
                                >
                                    <Save size={16} className="text-gray-400 group-hover:text-purple-400 transition-colors" />
                                    라이브러리에 저장
                                </button>
                            ) : (
            ...
            <motion.div
                ...
                className="w-full min-h-screen max-w-[1200px] mx-auto pt-44 pb-24 px-4 flex flex-col items-center justify-center"
            >
                <div className={`w-full max-w-lg overscroll-none rounded-2xl overflow-hidden shadow-2xl border border-white/10 relative ${type === 'webtoon' ? 'aspect-auto' : 'aspect-[3/4]'}`}>
                    {type === 'webtoon' ? (
                        <img
                            src={mediaUrl}
                            alt="Dream Webtoon"
                ...
                <div className="flex justify-center gap-8 py-4 mt-6">
                    <button
                        onClick={handleFavorite}
                        className="group flex flex-col items-center gap-2 text-gray-400 hover:text-white transition-colors"
                    >
                        <div className={`p-4 rounded-full bg-white/5 border border-white/10 group-hover:bg-white/10 group-hover:scale-110 transition-all ${isFavorite ? 'text-pink-500 border-pink-500/50 bg-pink-500/10' : ''
                            }`}>
                        ...
                        >
                            ...
                            <div className="relative z-10 text-center">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-4">
                                    ...
                                    >
                                        {shareCopied ? <Check size={24} className="mb-2 text-green-400" /> : <Copy size={24} className="mb-2" />}
